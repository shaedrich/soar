...
var ExplainExtra = map[string]string{
	"Using temporary":                   "Indicates that MySQL uses temporary tables when sorting query results. Commonly used in order by and group by queries.",
	"Using filesort":                    "MySQL will sort the results using an external index, rather than reading from the table in index order. Sorting may be done in memory or on disk. Sorting in MySQL that cannot be done using an index is called 'file sorting'.",
	"Using index condition":             "A new feature added after version 5.6 (Index Condition Pushdown), Using index condition will conditionally filter the index first, find all the rows that match the index condition after filtering the index, and then filter those rows with other conditions in the WHERE clause.",
	"Range checked for each record":     "MySQL did not find good indexes that could be used, but found that if the column values from the preceding table were known, it was possible that some of the indexes could be used.",
	"Using where with pushed condition": "This is a message that appears only in the NDBCluster storage engine, with condition pushdown optimization turned on to make it possible to use it.",
	"Using MRR":                         "Optimized at the I/O level using MRR Optimization to reduce I/O overhead.",
	...
	"Impossible WHERE noticed after reading const tables": "Queried all const (and system) tables, but found that the WHERE query does not work.",
	"Using where":                              "The WHERE condition is used to filter out matches to the next table and return them to the client. Unless a full table scan was done on purpose, a join type of ALL or index, and no Using Where in the value of the Extra column, may be problematic for this query.",
	"Using join buffer":                        "Finds data from existing joins that has been read into the cache, and completes the join to the current table through the cache.",
	"Using index":                              "The columns can be retrieved from the table simply by indexing them, without having to read the actual rows. This strategy is used to optimize queries that use column values that are only part of a simple index.",
	"const row not found":                      "An empty table does something like SELECT ... FROM tbl_name.",
	...
	"Full scan on NULL key":                    "An optimization in subqueries, often used when null values cannot be accessed through the index.",
	"Impossible HAVING":                        "The HAVING condition has no effect, and returns the result set of the existing query.",
	"Impossible WHERE":                         "WHERE filtering has no effect, and you end up with a full table scan.",
	"LooseScan":                                "Use the semi-connected LooseScan strategy.",
	"No matching min/max row":                  "No rows satisfy the query, e.g. SELECT MIN(...) FROM ... WHERE condition.",
	"no matching row in const table":           "For join queries, the columns do not satisfy the conditions of the unique index or the table is empty.",
	"No matching rows after partition pruning": "For DELETE or UPDATE, the optimizer finds nothing to delete or update after partitioning. Similar to the query Impossible WHERE.",
	"No tables used":                           "The query does not have a FROM clause, or has a FROM DUAL clause.",
	"Not exists":                               "MySQL can optimize a LEFT JOIN query and found no more rows after the rows that match the LEFT JOIN criteria.",
	...
	"Select tables optimized away":             "By using indexes only, the optimizer may return only one row from the aggregate function result. For example, optimizing MIN/MAX operations based on indexes in the absence of a GROUP BY clause, or optimizing COUNT(*) operations for the MyISAM storage engine does not have to wait until the execution phase to perform calculations, but rather the optimization is done at the query execution plan generation phase.",
	"Using intersect":                          "Enabled index merge, i.e., perform conditional scans on multiple indexes and then merge their respective results, using the algorithm: index_merge_intersection",
	"Using union":                              "Enabled index merge, i.e.: conditional scanning of multiple indexes separately, and then merging their respective results, using the algorithm: index_merge_union",
	"Using sort_union":                         "Turn on index merge, i.e., perform a conditional scan on multiple indexes and then merge their respective results, using the algorithm: index_merge_sort_union",
}
